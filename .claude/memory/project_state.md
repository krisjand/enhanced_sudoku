...
## In progress
- PR #60: corpus generator improvements (startup logging, write/print every 500 generated puzzles, fix allMet legendary skip) — open, not yet merged

## Corpus files situation
- Corpus files are at `/home/kristian-joten-andersen/git/puzzle_corpus/` — OUTSIDE the repo
- Default path in the script is `../../puzzle_corpus` from `backend/` which resolves to that external path
- Correct default should be `../puzzle_corpus` (one level up from backend/ = project root puzzle_corpus/)
- Sandbox now blocks access to the external path — user must move files into `puzzle_corpus/` inside the repo first
- Once files are in the right place: commit them, fix the default path in the script, then merge PR #60

## Next up: move corpus files into repo, fix default -out path in script, merge PR #60
