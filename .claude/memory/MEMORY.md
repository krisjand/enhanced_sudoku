...
- [Flutter Formatting](feedback_flutter_formatting.md) — run `dart format lib/` before every commit; CI enforces it and will fail unformatted PRs
- [Frontend Architecture](frontend_architecture.md) — Riverpod + go_router; feature-based layout; SudokuGrid thin widget + overlay pattern; conflict-only validation; auto-fill notes; forced chains as modal overlay; backend URL configurable (default localhost:8080); owner merges PRs; manual acceptance once game board is playable
